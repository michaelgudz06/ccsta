-- Migration 044: copy fixes to the customer confirmation email built in
-- migration 043. Two changes, both scoped to submit_quote's CUSTOMER email
-- (v_body_text / v_body_html) only:
--
-- 1. The "what happens next" line no longer commits to a specific
--    turnaround time ("usually within one business day") -- replaced with
--    "We'll get back to you as soon as possible."
-- 2. Every em dash (—) in the actual email copy is removed, reworded with
--    commas/periods instead (intro sentence, the "continuous" shuttle
--    note, the contact-us line, and the plain-text signature delimiter).
--    The '—' used internally as a blank-value placeholder (e.g. when a
--    field is unexpectedly empty) is changed to 'TBD', matching the
--    wording already used for a blank trip date in this same function.
--
-- NOT changed: validation, inserts, and the admin "new quote request"
-- alert email are byte-for-byte identical to migration 043 -- the admin
-- email has its own separate '—' placeholders (school/destination/trip
-- date fallbacks) that are intentionally left alone, since this migration
-- only touches the customer-facing email.
