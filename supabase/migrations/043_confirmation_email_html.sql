-- Migration 043: branded HTML customer confirmation email.
--
-- Scope: ONLY the customer-facing "we received your quote request" email
-- (queued from submit_quote). Every other email in the system (the admin
-- new-quote alert, approve_quote, reject_quote, resolve_cancellation_request)
-- is untouched and stays plain-text for now -- matches the priority order
-- Melody gave (this submission email is the one that matters most).
--
-- 1. notification_log gets a new nullable body_html column. Nullable so
--    every existing/untouched email keeps inserting body-only and stays
--    plain text -- no other function needs to change.
-- 2. _html_escape(): customer-entered free text (school name, destination
--    name/address, stop addresses, contact name) is about to be
--    interpolated into real HTML for the first time. Unescaped, a stray
--    '&' or '<' in a school name breaks the layout, or worse, injects
--    markup into the customer's own email. This didn't matter for plain
--    text and is a genuinely new consideration.
-- 3. _queue_email() gains a 5th optional parameter (p_body_html DEFAULT
--    NULL), appended at the end. Postgres treats a trailing-parameter
--    change as a NEW overload rather than a true replace, so the old
--    4-arg signature is dropped first (see comment at that DROP below) --
--    every existing 4-arg call site (approve_quote, reject_quote,
--    resolve_cancellation_request, submit_quote's own admin alert) still
--    resolves fine against the new function via its trailing defaults,
--    so none of them need to change.
-- 4. submit_quote: only the customer-confirmation email-building section
--    changes. Builds a trip-type-adaptive summary (destination for
--    two-way/one-way/shuttle, a numbered stop list for multi-destination,
--    a run list for shuttle) in both plain text and HTML, using the same
--    v_shuttle_runs/v_stops/v_departure/v_return data the function already
--    parses for the inserts above it. Everything else in the function
--    (validation, inserts, the admin alert email) is byte-for-byte
...
REVOKE EXECUTE ON FUNCTION public._html_escape(text) FROM PUBLIC, anon, authenticated;

-- CREATE OR REPLACE cannot add a trailing parameter to an existing function
-- signature -- Postgres would treat (text,text,text,uuid,text) as a NEW
-- overload sitting alongside the old (text,text,text,uuid), not a true
-- replace. Drop the old signature first so there's exactly one
-- _queue_email left; every existing 4-arg call site still resolves fine
-- against the new function via its trailing defaults.
DROP FUNCTION IF EXISTS public._queue_email(text, text, text, uuid);
...
BEGIN
  ...
  INSERT INTO notification_log (type, recipient, subject, body, body_html, quote_id)
  VALUES ('email', btrim(p_recipient), p_subject, p_body, p_body_html, p_quote_id);
EXCEPTION WHEN OTHERS THEN
  NULL; -- a notification problem must never block the booking action itself
END;
$$;

REVOKE EXECUTE ON FUNCTION public._queue_email(text, text, text, uuid, text) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  v_school_esc := _html_escape(COALESCE(trim(p_data->>'school_name'), '—'));
  v_trip_date_fmt := CASE
    WHEN NULLIF(p_data->>'trip_date', '') IS NOT NULL
      THEN to_char((p_data->>'trip_date')::date, 'FMDay, FMMonth FMDD, YYYY')
    ELSE 'TBD'
  END;
  v_depart_fmt := CASE WHEN v_departure IS NOT NULL THEN to_char(v_departure, 'FMHH12:MI AM') ELSE '—' END;
  v_return_fmt := CASE WHEN v_return IS NOT NULL THEN to_char(v_return, 'FMHH12:MI AM') ELSE '—' END;
  ...
  -- Main detail rows: School / [Destination] / Trip date / Times / Group size / Pickup.
  ...
  v_detail_html :=
    format('<tr><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:13px; color:#6b7280; width:120px; vertical-align:top;">School</td><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:14px; color:#1f2b4d; vertical-align:top;">%s</td></tr>', v_school_esc)
    || CASE WHEN v_trip_type IN ('two_way', 'one_way', 'shuttle') THEN
         format('<tr><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:13px; color:#6b7280; vertical-align:top;">Destination</td><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:14px; color:#1f2b4d; vertical-align:top;">%s<br><span style="color:#6b7280; font-size:13px;">%s</span></td></tr>',
           _html_escape(COALESCE(p_data->>'destination_name', '—')), _html_escape(COALESCE(p_data->>'destination_address', '')))
       ELSE '' END
    ...
    || format('<tr><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:13px; color:#6b7280; vertical-align:top;">%s</td><td style="padding:8px 0; border-bottom:1px solid #eceef2; font-size:14px; color:#1f2b4d; vertical-align:top;">%s &rarr; %s%s</td></tr>',
         ...
         CASE WHEN v_trip_type = 'shuttle' THEN '<br><span style="color:#6b7280; font-size:13px;">continuous — see runs below</span>' ELSE '' END)
    ...
    || format('<tr><td style="padding:8px 0; font-size:13px; color:#6b7280; vertical-align:top;">Pickup</td><td style="padding:8px 0; font-size:14px; color:#1f2b4d; vertical-align:top;">%s</td></tr>', _html_escape(COALESCE(p_data->>'pickup_address', '—')));

  v_detail_text :=
    '  School:      ' || COALESCE(trim(p_data->>'school_name'), '—') || E'\n'
    || CASE WHEN v_trip_type IN ('two_way', 'one_way', 'shuttle') THEN
         '  Destination: ' || COALESCE(p_data->>'destination_name', '—') || ', ' || COALESCE(p_data->>'destination_address', '—') || E'\n'
       ELSE '' END
    ...
    || '  Pickup:      ' || COALESCE(p_data->>'pickup_address', '—') || E'\n';
