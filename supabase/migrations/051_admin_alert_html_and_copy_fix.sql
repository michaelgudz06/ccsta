-- Migration 051: branded HTML admin "new quote request" alert + a customer
-- confirmation-email copy fix, both in submit_quote.
--
-- Based on migration 047's body (the latest submit_quote -- confirmed via
-- pg_get_functiondef-style migration audit in this session; 048 only
-- touches edit_own_quote). Two changes, no logic/schema touched:
--
-- 1. Customer confirmation email ("What happens next" line): was a vague
--    placeholder ("We'll get back to you as soon as possible") that never
--    actually matched Mila's approved mockup. Now matches it exactly:
--    "Melody will review your request and follow up with your official
--    price, usually within one business day." Both v_body_text and
--    v_body_html updated.
--
-- 2. Admin "new quote request" alert: was bare plain text (no HTML at all,
--    only 4 args to _queue_email -- no body_html param even passed) with
--    just quote/org/destination/date/rider-count. Now a branded HTML+text
--    email matching the same visual template as the customer email,
--    reusing the same detail-row/runs/stops building blocks (no duplicated
--    trip-detail logic), plus a Contact row (name/phone/email) the
--    customer email doesn't need shown back to itself. Admin-appropriate
--    framing: no customer greeting, no "contact us if this looks wrong"
--    line (doesn't apply when Melody is the recipient), CTA links to
--    /admin instead of /portal, no signature footer (Melody isn't sending
--    this to herself).
--
--    Row order changes for BOTH recipients (this is intentional, per the
--    approved mockup): Trip date / Trip type / Organization / Times /
--    Pickup / [Runs or Stops] / Drop off / Students, plus Contact for the
--    admin only. So "Destination" is renamed "Drop off" and moves below
--    the runs/stops block, "Group size" is renamed "Students" and becomes
--    the last customer row, and a new "Trip type" row is added. The
--    customer email's output is therefore NOT identical to 047 -- the
--    reorder and relabeling are the point.
--
--    Mechanically: the shared rows live in an "upper" chunk (Trip date /
--    Trip type / Organization / Times / Pickup), all carrying a
--    border-bottom since more rows always follow them now. The rows that
--    differ per recipient are built separately -- the customer's Students
--    row has NO border-bottom (it's the last one), while the admin's DOES,
--    because a Contact row follows it there.
--
...
BEGIN
  ...
  -- Shared detail rows (customer + admin), fixed order (migration 051):
  -- Trip date / Trip type / Organization / Times / Pickup. All carry a
  -- border-bottom — Pickup is no longer the last row in either email, since
  -- Drop off/Students (and Contact, for admin) now follow it below the
  -- runs/stops block.
  v_detail_html_upper :=
  ...
  -- Drop off (was "Destination") — shared, still hidden for
  -- multi_destination. Now sits after the runs/stops block and always
  -- carries a border-bottom, since Students follows it in both emails.
  ...
  -- Students (was "Group size") — the new last row for the customer email
  -- (no border-bottom); admin's Contact row follows it, so admin's keeps one.
  ...
  -- Customer's Drop off + Students block (Pickup already lives in the
  -- shared upper block above now).
  ...
  -- (multi-destination). Empty for two-way/one-way. Shared by customer +
  ...
  -- Customer confirmation email body (migration 051: "What happens next"
  -- copy fixed to match the approved mockup — was a vague placeholder).
  v_body_text :=
    'Hi ' || v_greeting || ',' || E'\n\n'
  ...
  -- ── Build the admin "new quote request" alert (text + HTML) — new in
  -- migration 051. Same template as the customer email, retargeted: no
  -- greeting, adds a Contact row, CTA goes to the dispatch dashboard,
  -- no "contact us if wrong" line, no signature footer.
  ...
  -- Admin "new quote request" alert — now branded HTML + text (migration
  -- 051), was plain text with no body_html param at all before.
  PERFORM _queue_email(
    _admin_email(),
