-- Migration 046: server-side trip-date bounds (must not be in the past,
-- capped 2 years in the future) on both submit_quote and edit_own_quote.
--
-- Client-side validation already blocks this in quote.tsx (validateAll),
-- but that's UX only, not enforcement -- a direct RPC call bypasses it
-- entirely. This adds the real guard.
--
-- submit_quote: based on migration 044's current body (the latest --
-- 043/044 already modified this function for the HTML confirmation email;
-- this migration must build on THAT body, not the older 038 one, or it
-- would regress the email work). Only change: two new RAISE EXCEPTION
-- checks right after the multi_trip guard, before anything else is parsed.
--
-- edit_own_quote: based on migration 042's current body (unchanged since).
-- The check applies to v_new_trip_date -- the function's own variable for
-- "the date this edit will actually save," which already accounts for
-- carrying forward the existing date when the customer doesn't change it.
-- This is additive to the existing 7-day-lock rule, not a replacement for
-- it: that rule already blocks editing a quote whose current trip_date is
-- already in the past (unless pushing later), so the only new case this
-- catches is a "later than before, but still somehow in the past" date,
-- plus the far-future cap, which the 7-day rule doesn't cover at all.
--
-- 2-year cap is generous for legitimate long-lead bookings while catching
-- an obvious typo'd year; revisit if that's ever too tight for a real trip.
--
...
BEGIN
  ...
  v_detail_text :=
    '  School:      ' || COALESCE(trim(p_data->>'school_name'), 'TBD') || E'\n'
    || CASE WHEN v_trip_type IN ('two_way', 'one_way', 'shuttle') THEN
  ...
  -- Admin "new quote request" alert -- UNCHANGED from migration 043/038.
  PERFORM _queue_email(
    _admin_email(),
